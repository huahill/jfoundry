...
- `jfoundry-spring` is the Spring runtime integration layer. Put Spring Framework adapters under `jfoundry-runtime-integrations/jfoundry-spring/runtime`, Spring Boot auto-configuration under `jfoundry-runtime-integrations/jfoundry-spring/autoconfigure/jfoundry-spring-boot-autoconfigure`, and Spring Boot starters under `jfoundry-runtime-integrations/jfoundry-spring/starters`. Spring-side wrappers may adapt Spring clients such as `KafkaTemplate` or `RabbitTemplate` to core SPI interfaces, but core and infrastructure modules must not require those Spring clients.
