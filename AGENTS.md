...
Keep `main` history linear. Do not use `git merge` to integrate completed work into `main`; use a pull request with GitHub's `Rebase and merge` strategy, or cherry-pick an explicitly selected ordered set of commits when that is the intended integration method. When synchronizing a feature branch with `main`, always use `git rebase origin/main` (or an equivalent rebase onto the current `main`); do not create a merge commit for routine synchronization. Do not rewrite already-pushed history unless the user explicitly authorizes it; when authorized, use `git push --force-with-lease`, not `--force`.

### Branch Workflow

Before editing or committing, confirm that the checkout is not `main`. Fetch the current remote refs and create a short-lived branch from `origin/main`, using the `codex/<scope>` naming convention unless a different branch name is explicitly requested. Keep all repository changes on that branch; never commit directly on `main`. When bringing new `main` changes into an existing feature branch, fetch first and rebase with `git rebase origin/main`; resolve conflicts and run the relevant checks before pushing. Rewriting an already-pushed feature branch for this rebase requires explicit user authorization and must use `git push --force-with-lease`. On the first push, set the upstream explicitly with `git push -u origin <branch>` so later pushes target the feature branch.
