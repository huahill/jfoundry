...
- Maven 4 final for Maven Central publication. Maven 3 is not a supported JFoundry source-build or release runtime.
...
directly from the Mason workspace. The workflow requires the plugin to report a Central
...
JFoundry uses Maven 4's official Consumer POM transformation because Maven Central validates the POM
that is deployed, rather than the build-time inheritance model. The source `pom.yaml` files remain
maintainable parent/BOM-based models; Maven 4 produces flattened Consumer POMs for child artifacts.
The workflow archives those transformed POMs and checks them with Maven 4 before publication.
...
not been validated. Once Maven 4 final is available, the same wrapper executes the standard `deploy`
lifecycle directly from the YAML reactor. No Maven 3 bridge or explicit Central goal is retained.
...
It records the existing publication, retains Maven 4 workspace evidence, and can complete a previously
...
the two historical BOM imports, and SCM tag `v1.0.0`. Maven 4 signs and deploys only that
