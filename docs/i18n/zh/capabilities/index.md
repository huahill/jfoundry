...
| Web | HTTP API 需要 RFC 9457 问题响应，或者 Spring 应用需要显式选择的出站 `RestClient` 支持。 | Problem Details：`jfoundry-webmvc-spring-boot-starter`；HTTP 客户端：`jfoundry-web-spring` | Problem Details：`jfoundry-web-quarkus-runtime` | Problem Details：`jfoundry-web-helidon-runtime` | [Web](web.md) |
