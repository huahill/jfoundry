...
| 领域事件派发 | 模型需要记录显式业务事实，并在进程内处理。 | `jfoundry-domain-event-spring-boot-starter` | `jfoundry-domain-event-quarkus-runtime` | `jfoundry-domain-event-helidon` | [领域事件](../modeling/domain-event.md) |
...
| 请求关联 | HTTP 应用需要统一的 `X-Request-Id`、请求上下文和日志关联。 | 与 Web MVC 入口一起选择 | 与 Web 入口一起选择 | 与 Web 入口一起选择 | [请求关联](request-correlation.md) |
