...
公开版本承诺、发布候选版本验收条件和受支持的运行时范围，由
[发布支持策略](../../../release/support-policy.md)定义。不可变发布工件的供应链控制和保留证据，由
[供应链安全](../../../release/supply-chain-security.md)定义。
...
- Java 25 业务应用。
- 受支持的 Spring Boot 与 Spring Framework 平台线，包括 JVM 中间件集成，以及基础运行时、
...
- 受支持 Quarkus 平台线的 CDI 发现与 Jakarta Transactions `TransactionRunner` 集成，已具备 JVM 运行时集成验证
  和原生镜像 CI 验证任务。
- 受支持 Helidon MP 平台线的 CDI/JTA、JPA 装配、JPA Outbox/Inbox 存储、调度与 JAX-RS Problem Details。
...
截至评估日期，稳定版本 `1.0.1` 已发布；仓库已经具备发布自动化和明确的
[兼容矩阵](../../../release/compatibility.md)。生产项目不应依赖持续变化的 SNAPSHOT，
而应使用正式发布版本或由组织内部治理的不可变构建。
