...
| Application transactions | A use case needs a runtime transaction boundary. | `jfoundry-transaction-spring-boot-starter` | `jfoundry-quarkus-runtime` | `jfoundry-transaction-helidon` | [Application Transactions](application-transactions.md) |
...
| Reliable messaging | A message must be recorded transactionally, dispatched later, or processed idempotently. | Outbox capability plus an explicit store and sender, or Inbox capability plus a store | `jfoundry-outbox-quarkus-runtime` or `jfoundry-inbox-jpa-quarkus-runtime` | `jfoundry-outbox-helidon-runtime` or `jfoundry-inbox-jpa-helidon-runtime` | [Reliable Messaging: Outbox And Inbox](reliable-messaging.md) |
