...
| `jfoundry.web.rest-client.logging.format` | `HUMAN` | Selects `HUMAN` Feign-style `-->` / `<--` lines with bodies on one line and `END HTTP` closers, or `INLINE` one-line `key=value` events, for outbound `RestClient` logs. |
| `jfoundry.web.rest-client.logging.included-headers` | `accept`, `authorization`, `content-type`, `content-length`, `location`, `x-request-id` | Header names included in outbound `RestClient` logs at `HEADERS` or `FULL`. A configured list replaces the default. Use `*` to include every header after redaction. |
