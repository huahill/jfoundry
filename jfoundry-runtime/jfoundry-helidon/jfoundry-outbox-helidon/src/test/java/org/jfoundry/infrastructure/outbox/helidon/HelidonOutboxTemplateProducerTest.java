...
class HelidonOutboxTemplateProducerTest {

    @Test
    void exposesReplaceableCdiDefaultsWithoutApplicationScopedNativeProxies() {
        assertThat(HelidonOutboxTemplateProducer.class.isAnnotationPresent(Alternative.class)).isTrue();
        assertThat(HelidonOutboxTemplateProducer.class.isAnnotationPresent(Priority.class)).isTrue();
    }

    @Test
    void createsTheDefaultJacksonPayloadSerializer() {
        assertThat(new HelidonOutboxTemplateProducer().payloadSerializer().serialize(new Event("order-1")))
                .contains("order-1");
    }
    ...
    void applicationPayloadSerializerReplacesTheEnabledDefaultProducer() {
        ...
        try {
            try (SeContainer container = SeContainerInitializer.newInstance()
                    .disableDiscovery()
                    .addBeanClasses(HelidonOutboxTemplateProducer.class, ApplicationPayloadSerializer.class)
                    .initialize()) {
