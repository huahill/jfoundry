...
class DomainEventExternalizationIntegrationTest {
    ...
    void applicationServiceEventEndsUpAtMessageSender() {
        System.out.println("BEANS=" + java.util.Arrays.stream(applicationContext.getBeanDefinitionNames())
                .filter(name -> name.toLowerCase().contains("domain") || name.toLowerCase().contains("outbox"))
                .toList());
        useCase.create();
