...
/// Records Quarkus REST server and MicroProfile REST Client activity at the configured detail level.
@Provider
@PreMatching
...
public final class HttpLoggingProvider implements ContainerRequestFilter, ContainerResponseFilter,
    ...
    public void filter(ContainerRequestContext request) {
        ...
        if (level.includesBodies()) {
            var body = bodyLog(captured -> LOG.debug(
                    "HTTP server request body: method={}, uri={}, body={}", state.method(), state.uri(),
    ...
    public void filter(ContainerRequestContext request, ContainerResponseContext response) {
        ...
        safely(() -> {
            ...
            if (state.level().includesHeaders()) {
                LOG.debug("HTTP server response: method={}, uri={}, status={}, headers={}, durationMs={}",
                        state.method(), state.uri(), response.getStatus(),
                        HttpLoggingPolicy.describeHeaders(response.getStringHeaders()), durationMs);
            } else {
                LOG.debug("HTTP server response: method={}, uri={}, status={}, durationMs={}",
                        state.method(), state.uri(), response.getStatus(), durationMs);
            }
        });
        if (state.level().includesBodies()) {
            var body = bodyLog(captured -> LOG.debug(
                    "HTTP server response body: method={}, uri={}, status={}, body={}", state.method(), state.uri(),
    ...
    public void filter(ClientRequestContext request) {
        ...
        safely(() -> {
            if (level.includesHeaders()) {
                LOG.debug("HTTP client request: method={}, uri={}, headers={}", state.method(), state.uri(),
                        HttpLoggingPolicy.describeHeaders(request.getStringHeaders()));
            } else {
                LOG.debug("HTTP client request: method={}, uri={}", state.method(), state.uri());
            }
        });
        if (level.includesBodies()) {
            var body = bodyLog(captured -> LOG.debug(
                    "HTTP client request body: method={}, uri={}, body={}", state.method(), state.uri(),
                    JaxRsHttpLoggingSupport.describeBody(request.getMediaType(), captured)));
    ...
    public void filter(ClientRequestContext request, ClientResponseContext response) {
        ...
        safely(() -> {
            var durationMs = elapsedMillis(state.startedAt());
            if (state.level().includesHeaders()) {
                LOG.debug("HTTP client response: status={}, headers={}, durationMs={}", response.getStatus(),
                        HttpLoggingPolicy.describeHeaders(response.getHeaders()), durationMs);
            } else {
                LOG.debug("HTTP client response: status={}, durationMs={}", response.getStatus(), durationMs);
            }
        });
        if (state.level().includesBodies()) {
            var body = bodyLog(captured -> LOG.debug(
                    "HTTP client response body: status={}, body={}", response.getStatus(),
    ...
    private void logServerRequest(ContainerRequestContext request, ServerState state) {
        safely(() -> {
            if (state.level().includesHeaders()) {
                LOG.debug("HTTP server request: method={}, uri={}, headers={}", state.method(), state.uri(),
                        HttpLoggingPolicy.describeHeaders(request.getHeaders()));
            } else {
                LOG.debug("HTTP server request: method={}, uri={}", state.method(), state.uri());
            }
        });
    }
    ...
    private static HttpLoggingLevel configuredLevel(String name, HttpLoggingLevel defaultValue) {
        try {
            ...
            safely(() -> LOG.debug("HTTP logging configuration could not be read: property={}", name));
            return defaultValue;
        }
    }
